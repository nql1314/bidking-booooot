...
def normalize_line(line: str) -> str:
    ...
    line = line.replace("平均占用的格子数量", "平均格数")
    return line
...
def parse_green_white_avg(line: str) -> float | None:
    patterns = [
        r"所有白色和绿色品质藏品平均格(?:子)?数(?:约为|为|约)?([\d,]+(?:\.\d+)?)(?:格)?",
        r"白色和绿色品质藏品平均格(?:子)?数(?:约为|为|约)?([\d,]+(?:\.\d+)?)(?:格)?",
        r"白绿平均格(?:子)?数(?:约为|为|约)?([\d,]+(?:\.\d+)?)(?:格)?",
        r"白\+绿平均格(?:子)?数(?:约为|为|约)?([\d,]+(?:\.\d+)?)(?:格)?",
    ]
    for pattern in patterns:
        match = re.search(pattern, line)
        if match:
...
def parse_color_avg(line: str) -> tuple[str, int | float] | None:
    if has_wg_phrase(line) or not is_likely_item_line(line):
        return None
    match = re.search(
        color_pattern() + optional_item_word_pattern() + r"平均格(?:子)?数(?:约为|为|约)?([\d,]+(?:\.\d+)?)(?:格)?",
        line,
    )
    if match:
...
def parse_random_pick_avg_value(line: str) -> tuple[int, float] | None:
    """随机选择的 n 件藏品平均价值约为 a（a 与各色「平均价值」同为点数口径）。"""
    match = re.search(
        r"随机选择(?:的)?(\d+)件藏品平均价值约(?:为)?([\d,]+(?:\.\d+)?)",
        line,
    )
    if match:
        return int(match.group(1)), normalize_number(match.group(2))
...
def parse_central_info(text: str) -> dict[str, Any]:
    ...
    for raw_line in lines:
        ...
        random_pick = parse_random_pick_avg_value(line)
        if random_pick is not None:
            pick_n, pick_avg = random_pick
            result["random_pick_count"] = pick_n
            result["random_pick_avg_price"] = pick_avg
            append_fact(result, "random_pick_count", pick_n, raw_line)
            append_fact(result, "random_pick_avg_price", pick_avg, raw_line)
            parsed_any = True
