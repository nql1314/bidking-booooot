...
``run_map_selection_transition`` / ``run_loop`` / ``run_aisha_loop``。

实现均在 ``_legacy_bot``；``run_aisha_loop`` 为清快照 + 强制 ``aisha_premium`` 的兼容入口。
"""
...
run_aisha_loop = getattr(_b, "run_aisha_loop", None)
