...
def main() -> int:
    ...
    parser.add_argument(
        "--full-client",
        action="store_true",
        help="Save the entire window client area; omit for central_info crop only.",
    )
    ...
    if args.full_client:
        ...
        payload = {
            "ok": True,
            "capture_mode": "full_client",
            "output": "",
            "window": info.__dict__,
            "client_size": [info.width, info.height],
            "crop_rect": None,
        }
        default_name = "window_capture_full_client.png"
    else:
        ...
        default_name = "window_capture_test.png"

    output = Path(args.output) if args.output else Path(__file__).resolve().parent / "automation_runs" / default_name
