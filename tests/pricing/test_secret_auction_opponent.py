...
def test_secret_opponent_round4_uses_round3_rank_and_cache() -> None:
    """第 4 回合：``bid_pre``=cache[3]，名次=prices[2]（PriceLog 第 3 轮，1 最好 ×1.0）。"""
    cfg = _minimal_config("941456831344888")
    snap = {
        "game_state": {
            "map_id": 4508,
            "players": {
                "941456831344888": {"name": "self", "prices": {"0": 3, "1": 3, "2": 1}},
                "111": {"name": "a", "prices": {"0": 1, "1": 2, "2": 4}},
            },
        },
    ...
    assert detail.get("rank_signal_round") == 2
