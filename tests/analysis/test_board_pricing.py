...
class BoardPricingTests(unittest.TestCase):
    ...
    def test_vacant_gold_tier_power_decay_on_est_orange(self) -> None:
        ...
        section = {"vacant_tier_cell_exponents": {"q5": 0.85, "q5+q6": 0.86, "q6": 0.7}}
        with patch.object(bp, "_pricing_section_for_map", return_value=section):
            p = bp.build_snapshot_pricing_dict(snap)
        t = float(p.get("total") or 0.0)
        kcw = float(p.get("known_contour_weighted_price") or 0.0)
        vac_adj = 2
        prem_gold = 100.0 * (vac_adj**0.85)
        prem_gr = 200.0 * (vac_adj**0.86)
        prem_red = 300.0 * float(vac_adj)
        base = t - kcw
        self.assertEqual(p.get("vacant_tier_exponent_q5"), 0.85)
        self.assertEqual(p.get("vacant_tier_exponent_q5_q6"), 0.86)
        self.assertEqual(p.get("vacant_tier_exponent_q6"), 1.0)
        self.assertEqual(p.get("est_orange"), int(round(base + prem_gold)))
        self.assertEqual(p.get("est_gold_red"), int(round(base + prem_gr)))
        self.assertEqual(p.get("est_red"), int(round(base + prem_red)))

    def test_red_vacant_stays_linear_when_q5_grid_count_known(self) -> None:
        from unittest.mock import patch
        ...
        snap = {"game_state": gs, "skill_logs": [], "map_id": 8888, "current_round": 5}
        csv = {
            "q4": 10.0,
            "q5": 1000.0,
            "q6": 50.0,
        ...
        raw = {"csv_quality_groups_avg_per_cell": csv, "event_stats": ev}
        section = {"vacant_tier_cell_exponents": {"q5": 0.85, "q6": 0.75}}
        with patch.object(bp, "_pricing_section_for_map", return_value=section):
            p = bp.build_snapshot_pricing_dict({**snap, "raw_pricing": raw})
        t = float(p.get("total") or 0.0)
        kcw = float(p.get("known_contour_weighted_price") or 0.0)
        vac_adj = int(p.get("vacant") or 0) + int(p.get("known_contour_weighted_cells") or 0)
        base = t - kcw
        linear_red = base + 50.0 * float(vac_adj)
        self.assertEqual(p.get("points"), int(round(linear_red)))
        self.assertEqual(p.get("vacant_tier_exponent_q6"), 1.0)
